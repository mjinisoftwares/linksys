          ...
          {servicesData?.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 rounded-lg border border-border bg-accent/10 hover:border-primary transition group"
                variants={itemVariants}
                whileHover={{ y: -8 }}
              >
                <motion.div
                  className="w-12 h-12 rounded-lg bg-accent/10 flex items-center justify-center mb-4 group-hover:bg-primary group-hover:text-white transition"
                ...
                <p className="text-muted-foreground text-sm mb-8 line-clamp-4">{service.summary}</p>

                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link
                    href={`/services/${service.slug}`}
                    className="inline-flex items-center gap-1 px-8 py-4 bg-accent text-white rounded-full hover:bg-primary/90 transition text-sm font-medium"
                  >
